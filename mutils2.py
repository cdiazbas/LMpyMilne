...
def notation(ju,lu,su,jl,ll,sl):
	lNotation = ['S','P','D','F']
	stringU = r'$^'+str(2*int(su)+1)+lNotation[int(lu)]+'_'+str(int(ju))+'$'
	stringL = r'$^'+str(2*int(sl)+1)+lNotation[int(ll)]+'_'+str(int(jl))+'$'
...
def paramLine(nlinea):
	...
	gu=g(ju,lu,su); gl=g(jl,ll,sl); gef = geff(ju,gu,jl,gl)
