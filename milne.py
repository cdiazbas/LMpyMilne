...
def stokesSyn(param,x,B,gamma,xi,vlos,eta0,a,ddop,S_0,S_1):
	...
	[[dpi,dsr,dsb],[spi,ssr,ssb],[ju,lu,su,jl,ll,sl,elem,l0,gu,gl,gef]] = deepcopy(param)
	...
	eta_I=1.0+0.5*eta0*(eta_p*(singamma**2.)+0.5*(eta_b+eta_r)*(1.+cosgamma**2.))
	# 2.- Elementos de dicroismo (pol dif en dif direcc) 
	eta_Q=0.5*eta0*(eta_p-0.5*(eta_b+eta_r))*(singamma**2.)*cos2xi
	eta_U=0.5*eta0*(eta_p-0.5*(eta_b+eta_r))*(singamma**2.)*sin2xi
	eta_V=0.5*eta0*(eta_r-eta_b)*cosgamma
	# 3.- Elementos de dispersion
	rho_Q=0.5*eta0*(rho_p-0.5*(rho_b+rho_r))*(singamma**2.)*cos2xi
	rho_U=0.5*eta0*(rho_p-0.5*(rho_b+rho_r))*(singamma**2.)*sin2xi
